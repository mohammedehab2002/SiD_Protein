...
@click.option('--arch',          help='Network architecture', metavar='ddpmpp|ncsnpp|adm',          type=click.Choice(['ddpmpp', 'ncsnpp', 'adm', 'proteina']), default='ddpmpp', show_default=True)
@click.option('--precond',       help='Preconditioning & loss function', metavar='vp|ve|edm',       type=click.Choice(['vp', 've', 'edm', 'proteina']), default='edm', show_default=True)

# Hyperparameters.
@click.option('--duration',      help='Training duration', metavar='MIMG',                          type=click.FloatRange(min=0, min_open=True), default=200, show_default=True)
@click.option('--batch',         help='Total batch size', metavar='INT',                            type=click.IntRange(min=1), default=512, show_default=True)
@click.option('--batch-gpu',     help='Limit batch size per GPU', metavar='INT',                    type=click.IntRange(min=1))
@click.option('--eval_batch',    help='Batch size for evaluation', metavar='INT',                   type=click.IntRange(min=1), default=10, show_default=True)
...
@click.option('--tmax',            help='the reverse sampling starting step corresoinding to the largest allowed noise level, in [0,1000]', metavar='FLOAT',  type=click.FloatRange(min=0, min_open=True), default=800, show_default=True)
...
# Parameters for Proteina
@click.option('--config_path',    help='Path to the configuration directory', metavar='DIR', type=str, default='training/proteina/configs/experiment_config/')
@click.option('--config_name',    help='Name of the configuration file', type=str, default='inference_ucond_200m_notri')
@click.option('--noise_scale',         help='Noise scale in SID', metavar='FLOAT', type=float, default=1.0, required=False)
@click.option('--min_n_res',         help='Minimum number of residues', metavar='INT', type=int, default=50, required=False)
@click.option('--max_n_res',         help='Maximum number of residues', metavar='INT', type=int, default=256, required=False)
...
Pretrained Diffusion Models for One-Step Generation".
    ...
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ProteinStructureDataset', path=opts.data, max_n_res=256, use_labels=opts.cond, xflip=opts.xflip, cache=opts.cache)
    ...
    dataset_name = 'Proteina'

    # Network architecture.
    if opts.arch == 'proteina':
        c.network_kwargs.update(config_path = opts.config_path, config_name = opts.config_name)

    # Preconditioning & loss function.
    if opts.precond == 'proteina':
        c.loss_kwargs.class_name = 'training.sid_loss.SID_ProteinaLoss'
        c.network_kwargs.class_name = 'training.networks.ProteinaWrapper'
    ...
    c.network_kwargs.update(nstep=opts.nstep)
    c.network_kwargs.update(noise_scale=opts.noise_scale)
    c.network_kwargs.update(eval_batch=opts.eval_batch)
    c.network_kwargs.update(t=opts.t, t_init=opts.t_init)
    c.network_kwargs.update(min_n_res=opts.min_n_res)
    c.network_kwargs.update(max_n_res=opts.max_n_res)
    ...
    if opts.seed is not None:
        c.update(seed=opts.seed)
    else:
        seed = torch.randint(1 << 31, size=[], device=torch.device('cuda'))
        torch.distributed.broadcast(seed, src=0)
        c.update(seed=int(seed))
    ...
    desc = f'{cond_str:s}-{opts.arch:s}-glr{opts.glr}-lr{opts.lr}-initsigma{opts.init_sigma}-gpus{dist.get_world_size():d}-alpha{c.alpha}-batch{c.batch_size:d}-tmax{c.tmax}-{dtype_str:s}-nstep{opts.nstep}'
