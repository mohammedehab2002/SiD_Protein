...
@click.option("--nstep", type=int, default=1, help="Number of diffusion steps.")
@click.option("--noise_scale", type=float, default=1.0, help="Noise scale at each step.")
def main(model_path, out_dir, lengths, conditional, num_batch, batch_size, seed, nstep, noise_scale):
    ...
    # G.load_state_dict(torch.load(model_path, weights_only=False))
    G = G.to(device)
    ...
    if conditional:
        ...
        eval_input_dir = generate_unconditional(out_dir, G, lengths, num_batch, batch_size, device, nstep, noise_scale, designability, seed)
...
def generate_unconditional(out_dir, G, lengths, num_batch, batch_size, device, nstep, noise_scale, designability, seed):
...
def save_structures(x_g, eval_coords_dir, eval_pdb_dir, n, niter, cath_code=None, sampled_cath_codes=None, designability=None, seed=None):
    ...
    for sample_idx in range(x_g.shape[0]):
        ...
        if cath_code is None:
            filename = f'{n}_{niter}_{sample_idx}_{seed}'
        else:
...
def create_directories(eval_input_dir):
    ...
    os.makedirs(os.path.join(eval_pdb_dir, "designable"), exist_ok=True)
    os.makedirs(os.path.join(eval_pdb_dir, "undesignable"), exist_ok=True)
