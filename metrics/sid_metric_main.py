...
from . import sid_scTM as scTM
from . import sid_mean_distance as mean_distance
from . import sid_scRMSD as scRMSD
...
@register_metric
def sctm(opts):
    scTM_score = scTM.compute_scTM(opts)
    return scTM_score

@register_metric
def mean_dist(opts):
    mean_dist = mean_distance.calculate_consecutive_distances(opts)
    return dict(mean_dist=mean_dist)

@register_metric
def scrmsd(opts):
    designability = scRMSD.compute_scRMSD(opts)
